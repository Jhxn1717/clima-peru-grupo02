...
export const WeatherCharts: React.FC<WeatherChartsProps> = ({ hourly, theme = 'dark' }) => {
  ...
  const tabs = [
    { id: 'temp', label: 'Temperatura', icon: Thermometer, color: 'text-amber-500 dark:text-amber-400' },
    { id: 'rain', label: 'Precipitación', icon: CloudRain, color: 'text-blue-500 dark:text-blue-400' },
    { id: 'humidity', label: 'Humedad', icon: Droplets, color: 'text-sky-500 dark:text-sky-400' },
    { id: 'wind', label: 'Viento', icon: Wind, color: 'text-teal-500 dark:text-teal-400' },
  ];
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="p-3 bg-white/95 dark:bg-slate-900/95 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl backdrop-blur-md text-xs space-y-1">
          <p className="font-bold text-sky-600 dark:text-sky-400">{label} hrs</p>
          ...
          {payload[0]?.payload?.description && (
            <p className="text-slate-500 dark:text-slate-400 text-[10px] pt-1 border-t border-slate-200 dark:border-slate-800">
              {payload[0].payload.description}
            </p>
          )}
        </div>
    ...
    <div className="glass-panel p-6 rounded-3xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Evolución de variables meteorológicas en las próximas 24 horas</p>
        </div>

        {/* Tab Buttons */}
        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900/80 p-1 rounded-2xl border border-slate-200 dark:border-slate-800 self-start sm:self-auto shadow-sm">
          {tabs.map((tab) => {
            const Icon = tab.icon;
        ...
        <ResponsiveContainer width="100%" height="100%">
          {activeTab === 'temp' ? (
            ...
            <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={gridStroke} vertical={false} />
              <XAxis dataKey="hour" stroke={axisStroke} tick={{ fontSize: 11 }} />
              <YAxis stroke={axisStroke} tick={{ fontSize: 11 }} unit="%" domain={[0, 100]} />
              <Tooltip content={<CustomTooltip />} />
            ...
            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <CartesianGrid strokeDasharray="3 3" stroke={gridStroke} vertical={false} />
              <XAxis dataKey="hour" stroke={axisStroke} tick={{ fontSize: 11 }} />
              <YAxis stroke={axisStroke} tick={{ fontSize: 11 }} unit=" km/h" />
              <Tooltip content={<CustomTooltip />} />
