          ...
          <div className="flex items-center bg-slate-900 p-1 rounded-2xl border border-slate-800 text-xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-xl font-semibold transition-all ${
                viewScope === 'national' ? 'bg-sky-500 text-white shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
              Nivel Nacional
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-xl font-semibold transition-all ${
                viewScope === 'city' ? 'bg-sky-500 text-white shadow-sm' : 'text-slate-400 hover:text-white'
              }`}
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <button
            ...
            className={`p-3 rounded-2xl border transition-all text-left ${
              filterSeverity === 'danger' ? 'bg-red-500/20 border-red-500' : 'bg-slate-900/60 border-slate-800 hover:border-slate-700'
            }`}
          >
            <div className="flex justify-between items-center">
              <span className="text-xs font-semibold text-red-400">Peligro</span>
              <AlertOctagon className="w-4 h-4 text-red-400" />
            </div>
            <div className="text-2xl font-extrabold text-white mt-1">{alertsData.danger_count}</div>
          </button>

          <button
            onClick={() => setFilterSeverity('warning')}
            className={`p-3 rounded-2xl border transition-all text-left ${
              filterSeverity === 'warning' ? 'bg-amber-500/20 border-amber-500' : 'bg-slate-900/60 border-slate-800 hover:border-slate-700'
            }`}
          >
            <div className="flex justify-between items-center">
              <span className="text-xs font-semibold text-amber-400">Advertencia</span>
              <AlertTriangle className="w-4 h-4 text-amber-400" />
            </div>
            <div className="text-2xl font-extrabold text-white mt-1">{alertsData.warning_count}</div>
          </button>

          <button
            onClick={() => setFilterSeverity('caution')}
            className={`p-3 rounded-2xl border transition-all text-left ${
              filterSeverity === 'caution' ? 'bg-yellow-500/20 border-yellow-500' : 'bg-slate-900/60 border-slate-800 hover:border-slate-700'
            }`}
          >
            <div className="flex justify-between items-center">
              <span className="text-xs font-semibold text-yellow-400">Precaución</span>
              <Info className="w-4 h-4 text-yellow-400" />
            </div>
            <div className="text-2xl font-extrabold text-white mt-1">{alertsData.caution_count}</div>
          </button>

          <button
            onClick={() => setFilterSeverity('all')}
            className={`p-3 rounded-2xl border transition-all text-left ${
              filterSeverity === 'all' ? 'bg-sky-500/20 border-sky-500' : 'bg-slate-900/60 border-slate-800 hover:border-slate-700'
            }`}
          >
            <div className="flex justify-between items-center">
              <span className="text-xs font-semibold text-sky-400">Todas</span>
              <Filter className="w-4 h-4 text-sky-400" />
            </div>
            <div className="text-2xl font-extrabold text-white mt-1">{alertsData.total_alerts}</div>
          </button>
        </div>
      )}
        ...
        {isLoading ? (
          ...
          filteredAlerts.map((alert) => {
            ...
            return (
              <div
                key={alert.id}
                className={`p-5 rounded-2xl border ${style.bg} transition-all space-y-3`}
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-xs text-slate-300 mt-1">
                        ...
                        {alert.department_name && <span className="text-slate-400">({alert.department_name})</span>}
                        {alert.region_natural && (
                          <span className="text-[10px] px-2 py-0.2 rounded-full bg-slate-900 text-slate-300">
                            {alert.region_natural}
                          </span>
                        )}
                      </div>
                  ...
                  {alert.city_name !== 'Territorio Nacional del Perú' && (
                    <button
                      ...
                      className="text-xs font-semibold text-sky-400 hover:text-sky-300 underline self-start sm:self-auto"
                    >
                      Consultar ciudad →
                    </button>
                  )}
                ...
                {/* Trigger metadata */}
                <div className="flex justify-between items-center text-[10px] text-slate-400 pt-1 border-t border-slate-800/40">
                  <span>Valor detectado: <strong className="text-slate-300">{alert.trigger_value}</strong></span>
                  <span>Umbral de activación: <strong className="text-slate-300">{alert.threshold}</strong></span>
                </div>
              </div>
            );
          })
