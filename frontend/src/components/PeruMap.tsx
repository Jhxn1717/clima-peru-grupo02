  ...
  return (
    <div className="glass-panel p-6 rounded-3xl space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">
              Mapa Interactivo Meteorológico del Perú
            </h3>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        {/* Map Canvas */}
        <div className="lg:col-span-3 h-[480px] rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 relative shadow-inner">
          <div ref={mapContainerRef} className="w-full h-full" />
          
          {/* Map Legend Floating */}
          <div className="absolute bottom-3 left-3 z-10 glass-panel p-2.5 rounded-xl text-[10px] space-y-1.5 max-w-[200px] border border-slate-200 dark:border-slate-700/80 shadow-md">
            <span className="font-bold text-slate-700 dark:text-slate-300 block">
              Escala de {metric === 'temperature' ? 'Temperatura (°C)' : metric === 'precipitation' ? 'Precipitación (mm)' : 'Índice UV'}
            </span>
            {metric === 'temperature' ? (
              <div className="flex items-center justify-between gap-1">
                <span className="text-cyan-600 dark:text-cyan-400">&le;12° Frío</span>
                <span className="text-emerald-600 dark:text-emerald-400">20° Templado</span>
                <span className="text-rose-600 dark:text-rose-400">&ge;28° Cálido</span>
              </div>
            ) : metric === 'precipitation' ? (
              <div className="flex items-center justify-between gap-1">
                <span className="text-slate-500 dark:text-slate-400">0mm Seco</span>
                <span className="text-blue-600 dark:text-blue-400">2-8mm Moderada</span>
                <span className="text-purple-600 dark:text-purple-400">&gt;8mm Fuerte</span>
              </div>
            ) : (
              <div className="flex items-center justify-between gap-1">
                <span className="text-emerald-600 dark:text-emerald-400">1-2 Bajo</span>
                <span className="text-amber-600 dark:text-amber-400">6-7 Alto</span>
                <span className="text-purple-600 dark:text-purple-400">11+ Extremo</span>
              </div>
            )}
          </div>
        </div>
