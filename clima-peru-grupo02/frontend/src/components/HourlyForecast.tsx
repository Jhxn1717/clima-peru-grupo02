  ...
  return (
    <div className="glass-panel p-6 rounded-3xl space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-slate-500 dark:text-slate-400">Desplazamiento horizontal</span>
      </div>
          ...
          return (
            <div
              ...
              className={`shrink-0 w-28 p-3.5 rounded-2xl flex flex-col items-center justify-between text-center transition-all ${
                isNow
                  ? 'bg-gradient-to-b from-sky-500/20 to-sky-100/80 dark:to-slate-900/90 border border-sky-500/50 shadow-md shadow-sky-500/10'
                  : 'glass-card hover:border-sky-400/50 dark:hover:border-slate-600/60'
              }`}
            >
              {/* Hour Label */}
              <span className={`text-xs font-semibold ${isNow ? 'text-sky-600 dark:text-sky-300 font-bold' : 'text-slate-500 dark:text-slate-400'}`}>
                {isNow ? 'Ahora' : item.hour_label}
              </span>

              {/* Weather Icon */}
              <div className="my-2.5 p-2 rounded-xl bg-slate-100 dark:bg-slate-800/60 shadow-inner">
              ...
              <div className="w-full mt-2 pt-2 border-t border-slate-200 dark:border-slate-800/80 space-y-1">
                <div className="flex items-center justify-center gap-1 text-[11px] text-blue-600 dark:text-blue-400 font-medium">
