  ...
  return (
    <div className="glass-panel p-6 rounded-3xl space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-slate-500 dark:text-slate-400">Rango térmico y lluvia</span>
      </div>
          ...
          return (
            <div
              ...
              className={`p-3.5 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 transition-all ${
                isToday
                  ? 'bg-sky-50/80 dark:bg-slate-800/80 border border-sky-400/40 dark:border-sky-500/30 shadow-sm'
                  : 'glass-card hover:bg-slate-50 dark:hover:bg-slate-800/50'
              }`}
            >
              {/* Day & Date */}
              <div className="w-full sm:w-36 flex items-center justify-between sm:justify-start gap-3">
                <div>
                  <span className={`text-sm font-bold block ${isToday ? 'text-sky-600 dark:text-sky-400' : 'text-slate-800 dark:text-white'}`}>
                ...
                <div className="sm:hidden flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-700 dark:text-slate-300 font-semibold">{Math.round(day.temp_max)}° / {Math.round(day.temp_min)}°</span>
                </div>
              </div>

              {/* Icon & Condition Description */}
              <div className="hidden sm:flex items-center gap-3 w-48">
                <div className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-900/60 shadow-sm">
                  <WeatherIcon name={day.weather_icon} size={24} />
                </div>
              ...
              <div className="flex items-center gap-4 w-36 text-xs">
                <div className="flex items-center gap-1 text-blue-600 dark:text-blue-400 font-medium" title="Probabilidad de lluvia">
                  <Droplets className="w-3.5 h-3.5" />
                  <span>{day.precipitation_probability_max}%</span>
                </div>
                <div className="flex items-center gap-1 text-amber-600 dark:text-amber-400 font-medium" title="Índice UV máximo">
              ...
              <div className="hidden sm:flex items-center gap-3 flex-1 max-w-xs">
                <div className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 text-xs font-bold w-10 text-right">
                  <ArrowDown className="w-3 h-3" />
                  <span>{Math.round(day.temp_min)}°</span>
                </div>

                <div className="flex-1 bg-slate-200 dark:bg-slate-900/90 rounded-full h-2 relative overflow-hidden">
                  <div
                ...
                <div className="flex items-center gap-1 text-rose-600 dark:text-rose-400 text-xs font-bold w-10">
