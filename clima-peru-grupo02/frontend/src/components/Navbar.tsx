...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const { user, status, logout, isAdmin, isSuperAdmin } = useAuth();
        ...
        <div className="flex items-center justify-between h-20 gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-2.5">
            ...
            <button
              ...
              disabled={isRefreshing}
              title="Descargar reporte PDF del pronóstico actual"
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-600 dark:text-red-400 text-xs font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
            >
              {isRefreshing
                ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" /><span className="hidden sm:inline">Generando...</span></>
            ...
            {status === 'authenticated' && user ? (
              <div className="relative" ref={userMenuRef}>
                <button
                  ...
                  className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-xl bg-white/90 hover:bg-slate-100 dark:bg-slate-900/80 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700/60 shadow-sm transition-colors"
                >
                  <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center text-white font-bold text-xs shrink-0">
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-52 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl z-50 py-1.5 overflow-hidden">
                    {/* User info */}
                    <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800">
                      <div className="text-sm font-semibold text-slate-800 dark:text-white truncate">{user.full_name}</div>
                      <div className="text-xs text-slate-400 truncate">{user.email}</div>
                      <div className="mt-1.5 flex flex-wrap gap-1">
                        {user.roles.map(r => (
                          <span key={r.id} className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium border ${r.is_super_admin ? 'bg-purple-500/15 text-purple-600 dark:text-purple-300 border-purple-500/30' : 'bg-sky-500/15 text-sky-600 dark:text-sky-300 border-sky-500/30'}`}>
                            {r.display_name}
                          </span>
                        ))}
                      </div>
                    ...
                    {(isAdmin() || isSuperAdmin()) && onOpenAdmin && (
                      <button
                        ...
                        className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-left"
                      >
                    ...
                    <div className="border-t border-slate-100 dark:border-slate-800 mt-1 pt-1">
                      <button
                        ...
                        className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors text-left"
                      >
              ...
              <button
                onClick={() => onOpenAuth?.()}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-sky-600 hover:bg-sky-500 text-white text-xs font-semibold transition-colors shadow-sm shadow-sky-600/20"
              >
